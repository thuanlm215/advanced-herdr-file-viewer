...
| `herdr` | The herdr CLI seam (`$HERDR_BIN_PATH`): read-only queries (list git worktrees / which workspaces have an active agent) plus best-effort host **layout** commands (`pane zoom --current --on`/`--off` for `Z`, and `pane split --current --direction down --cwd <path> --focus` for `G` / Open pane here). Neither touches file or git state; an absent or failing herdr degrades gracefully (git-only picker, in-pane zoom only, or an action notice). |
