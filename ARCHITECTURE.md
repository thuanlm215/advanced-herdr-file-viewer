...
A high-level map of how `advanced-herdr-file-viewer` is built, for contributors and for anyone
...
| --- | --- |
| `host` | The herdr boundary: parse the injected `HERDR_PLUGIN_CONTEXT_JSON` launch context, degrading to `{ cwd }` on anything malformed (never panics), plus the pane-local exact-root override used by Open workspace here. |
| `context` | The normalized `LaunchContext` the host hands to the resolver. |
| `root` | Resolve the tree root (normally git worktree top-level, else cwd); an explicit Open workspace here launch keeps its selected folder while retaining the enclosing repo's git facts. The re-root engine re-resolves the root and rebuilds the tree + git services in place when you switch worktrees. |
...
| `workspace_search` | The injected, read-only ripgrep seam and bounded full-text search state: literal smart-case search over a selected file/folder or the workspace root, streamed from `rg --json`, cancelled when superseded, and capped at 500 displayed records. |
...
| `controller` | Orchestrate intents → state changes; hold ephemeral session state, including the root-bound `AnnotationStore`; dispatch renders and ripgrep searches off-thread; map mouse events against fed-back geometry; and rebuild root-bound services on a worktree switch. Its owned annotation projection root-joins file targets, follows the applied `content_path` rather than the live cursor, and exposes merged line ranges only when the applied render carries a source map. Feature submodules are `mod`, `mouse`, `help`, `finder`, `workspace_search`, `picker`, `infile`, `lineselect`, `annotation`, `context_menu`, and `git_apply`. One `Modal` enum type-enforces exclusive input ownership. Only a successful root change clears annotations; failure and same-root paths preserve them. Quitting or switching worktree with a non-empty store raises the `Modal::DiscardConfirm` layer (for a quit, outside search/unzoom) rather than silently discarding it; its `y` proceeds only on a successful clipboard write, and a switch re-validates its held target before committing. |
...
| `config` | Load & resolve the read-only TOML config: path resolution (`$HERDR_PLUGIN_CONFIG_DIR`, else XDG fallback), defensive parse (malformed input degrades to defaults, never panics), and precedence (config > env > default) → the `EffectiveSettings` consumed at startup by `editor`, `render`, `opener`, `update`, and Herdr workspace launch/layout; its fixed-point `ViewerPaneRatio` validates the host pane share and derives cross-platform launcher argv. It also parses the `[keys]` remapping table into `KeySpec` (string-or-array) entries the `input` bindings resolver layers over the registry. Never writes the file. |
