#!/bin/sh
# fetch-or-build.sh — herdr [[build]] step for advanced-herdr-file-viewer.
#
# Fast path: download the prebuilt binary that matches THIS source's declared version + platform
# from the GitHub release, verify its SHA-256, and install it at target/release/advanced-herdr-file-viewer.
...
out="${FV_OUT:-$repo_root/target/release/advanced-herdr-file-viewer}"
...
build_from_source() {
  ...
  if ! have cargo; then
    echo "advanced-herdr-file-viewer needs Rust 1.96+ to build, but cargo was not found. Install Rust from https://rustup.rs then re-run: herdr plugin install $repo" >&2
    exit 1
  fi
...
fallback() {
  echo "advanced-herdr-file-viewer: $1 — building from source instead." >&2
...
echo "advanced-herdr-file-viewer: installed prebuilt v$version ($triple), verified SHA-256.$ahead_note"
exit 0
