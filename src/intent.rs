...
pub enum Intent {
    ...
    OpenWorkspace,
    /// Open a herdr terminal pane below the viewer, rooted at the selected directory (`G`).
    /// For a selected file, use its parent directory. Read-only w.r.t. files and git — invokes
    /// `herdr pane split --current --direction down --cwd <path> --focus`.
    OpenPaneHere,
    /// Open the context menu of actions for the selected tree node (`Space`). Read-only overlay.
    ShowContextMenu,
    ...
    OpenFinder,
    /// Search file contents with ripgrep (`F`), scoped to the selected file or directory.
    /// The modal can toggle to the whole workspace. Read-only; unavailable when `rg` is absent.
    OpenTextSearch,
...
mod tests {
    ...
    fn all_length_is_43() {
        assert_eq!(
            Intent::ALL.len(),
            43,
            "Intent::ALL must have exactly 43 variants"
        );
    }

    #[test]
    fn open_workspace_pane_and_context_menu_are_in_all() {
        assert!(
            Intent::ALL.contains(&Intent::OpenWorkspace),
            "Intent::ALL must contain OpenWorkspace"
        );
        assert!(
            Intent::ALL.contains(&Intent::OpenPaneHere),
            "Intent::ALL must contain OpenPaneHere"
        );
        assert!(
