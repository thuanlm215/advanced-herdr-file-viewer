...
pub fn run(open_flag: Option<String>) -> io::Result<()> {
    ...
    controller.set_workspace_searcher(Box::new(
        crate::workspace_search::LiveWorkspaceSearcher::default(),
    ));
...
fn event_loop(terminal: &mut DefaultTerminal, controller: &mut Controller) -> io::Result<()> {
    ...
    loop {
        ...
        if event::poll(TICK)? {
            match event::read()? {
                Event::Key(key)
                    if key.kind == KeyEventKind::Press && controller.workspace_search_open() =>
                {
                    let fx = controller.handle_workspace_search_key(key);
                    if fx.clear {
...
/// install dir (the binary runs from `<root>/target/<profile>/advanced-herdr-file-viewer`, with the
...
mod tests {
    ...
    fn bundled_style_is_found_in_the_executables_install_tree() {
        ...
        let exe = root.join("target/release/advanced-herdr-file-viewer");
