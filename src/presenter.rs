...
use crate::workspace_search::{SEARCH_LIMIT, WorkspaceMatch};
use ratatui::Frame;
...
pub struct ViewState {
    ...
    /// The tree's vertical scroll offset (first visible node index). Mouse-wheel and scrollbar
    /// input move this viewport independently of the selected node.
    pub tree_scroll: u16,
    /// Bring the selected tree row into view on this frame. Keyboard/programmatic selection sets
    /// this; direct viewport scrolling clears it so the selection may remain off-screen, matching
    /// conventional file explorers.
    pub tree_follow_selection: bool,
    ...
    /// Top-anchored ripgrep full-text search overlay (`F`).
    pub workspace_search: Option<WorkspaceSearchView>,
...
/// Draw model for ripgrep-backed full-text search.
pub struct WorkspaceSearchView {
    pub query: String,
    pub scope_label: String,
    pub workspace: bool,
...
fn finder_match_line(path: &str, selected: bool, icons: crate::config::TreeIcons) -> Line<'static> {
    ...
    let (glyph, glyph_color) = file_icon(path, icons);
    let name = path
        .file_name()
    ...
    let parent = path
        ...
        .map(|parent| sanitize_control(&parent.to_string_lossy()));
    let mut spans = vec![
        Span::styled(
            glyph,
...
fn draw_tree(frame: &mut Frame, area: Rect, state: &ViewState) {
    ...
    // (possibly shrunk) text rect. The vertical offset is independently controlled by the
    // controller; keyboard/programmatic selection can request one-frame cursor following. The
    // horizontal offset lets long / deeply-nested rows be read sideways (`H`/`L` scroll the tree;
    // ←/→ are expand/collapse in the tree).
...
/// `tree_inner.y + r` is index `r + tree_scroll`. `content_inner` is the content column interior.
/// `divider_x` is the draggable boundary column (wide layout only).
...
pub struct PaneGeometry {
    ...
    /// Click target for the `[Workspace]` / `[Selection]` scope button.
    pub workspace_search_scope_button: Option<Rect>,
...
pub fn geometry(area: Rect, state: &ViewState) -> PaneGeometry {
    ...
    // derived identically (over the reduced text height + requested viewport offset). Saturating casts:
    ...
    let (workspace_search_rows, workspace_search_scroll, workspace_search_scope_button) =
        match &state.workspace_search {
            Some(search) => {
                let layout = workspace_search_overlay_layout(area, search, state.tree_icons);
                (
                    layout.rows_area,
                    layout.offset.min(u16::MAX as usize) as u16,
                    layout.scope_button,
                )
            }
            None => (None, 0, None),
...
pub fn draw(frame: &mut Frame, state: &ViewState) -> (u16, u16) {
    ...
    if let Some(search) = &state.workspace_search {
        draw_workspace_search_overlay(frame, frame.area(), search, state.tree_icons);
    }
...
/// Resolve the tree viewport for this frame. Direct scrolling uses the stored offset even when
/// the selection is off-screen; keyboard/programmatic selection requests the minimal adjustment
/// needed to reveal it.
fn tree_scroll_offset(state: &ViewState, visible: usize) -> usize {
    if visible == 0 || state.nodes.len() <= visible {
        return 0;
    }
    let max_offset = state.nodes.len() - visible;
    let current = (state.tree_scroll as usize).min(max_offset);
    if state.tree_follow_selection {
        sticky_scroll_offset(state.selected, state.nodes.len(), visible, current)
    } else {
        current
    }
}
...
/// Shared fixed width for the file finder and full-text search overlays.
const SEARCH_OVERLAY_MAX_W: u16 = 100;
...
fn finder_overlay_layout(
    ...
    // Build match lines for horizontal-scroll measurement.
...
/// Draw the go-to-file finder as a top-anchored, bordered overlay on top of the columns (AC-1).
///
...
///
/// Reuses [`scroll_offset`], `PICKER_PADDING`, and the Scrollbar/Block primitives from the picker
/// overlay — no duplication of their internals.
...
const WORKSPACE_SEARCH_PROMPT: &str = "> ";
const WORKSPACE_SEARCH_PLACEHOLDER: &str = "Type to search with ripgrep…";
const WORKSPACE_SEARCH_FOOTER: &str = "Tab scope · ↑↓ move · Enter open · Esc close";

struct WorkspaceSearchLayout {
    popup: Rect,
    query_area: Rect,
    rows_area: Option<Rect>,
    offset: usize,
...
/// Top-anchored, fixed-width layout. Only the bottom edge grows as result rows are added.
fn workspace_search_overlay_layout(
    area: Rect,
    search: &WorkspaceSearchView,
    _icons: crate::config::TreeIcons,
) -> WorkspaceSearchLayout {
    let width = area.width.saturating_sub(2).min(SEARCH_OVERLAY_MAX_W);
    let x = area.x + (area.width.saturating_sub(width)) / 2;
    let status_rows = usize::from(
        search.matches.is_empty()
            && (!search.query.is_empty() || search.pending || search.error.is_some()),
    );
    let result_rows = if search.matches.is_empty() {
        status_rows
    } else {
        search.matches.len().saturating_mul(2)
    };
    let desired_inner_h = 1u16.saturating_add(result_rows.min(u16::MAX as usize) as u16);
    let desired_h = desired_inner_h
        .saturating_add(2)
        .saturating_add(PICKER_PADDING * 2);
    let height = desired_h.min(area.height);
    let popup = Rect::new(x, area.y, width, height);
    let inner = modal_frame().inner(popup);
    let query_area = Rect::new(inner.x, inner.y, inner.width, u16::from(inner.height > 0));
    let remaining = inner.height.saturating_sub(query_area.height);
    let rows_area = (remaining > 0 && result_rows > 0).then(|| {
        Rect::new(
            inner.x,
    ...
    let offset = rows_area.map_or(0, |rows| {
        let visible_records = (rows.height as usize / 2).max(1);
        scroll_offset(search.cursor, search.matches.len(), visible_records)
    });

    let button_label = if search.workspace {
        "[Selection]"
    } else {
        "[Workspace]"
    };
...
fn workspace_search_match_lines(
    hit: &WorkspaceMatch,
    query: &str,
    selected: bool,
    icons: crate::config::TreeIcons,
    ...
    header.push(Span::styled(
        format!("  {}:{}", hit.line, hit.column),
        row_style.add_modifier(Modifier::DIM),
    ));

    let preview = sanitize_control(&hit.preview);
    let base = Line::from(Span::raw(preview.clone()));
    let matches = crate::search::find_matches(query, std::slice::from_ref(&preview));
    let mut preview_line = crate::highlight::apply(&[base], &matches, usize::MAX)
        .into_iter()
        .next()
        .unwrap_or_default();
    preview_line.spans.insert(0, Span::raw("  "));

    [Line::from(header), preview_line]
}

fn draw_workspace_search_overlay(
    frame: &mut Frame,
    area: Rect,
    search: &WorkspaceSearchView,
    icons: crate::config::TreeIcons,
) {
    let layout = workspace_search_overlay_layout(area, search, icons);
    frame.render_widget(Clear, layout.popup);

    let title = format!("Search: {}", sanitize_control(&search.scope_label));
    let footer = if search.limited {
        format!("{SEARCH_LIMIT}+ matches — results limited")
    } else {
        WORKSPACE_SEARCH_FOOTER.to_string()
    };
    let block = modal_frame()
    ...
    if let Some(button) = layout.scope_button {
        frame.render_widget(
            Paragraph::new(if search.workspace {
                "[Selection]"
            } else {
                "[Workspace]"
            })
    ...
    if layout.query_area.height > 0 {
        let query = if search.query.is_empty() {
            ...
            Line::from(format!(
                ...
                sanitize_control(&search.query)
            ))
        };
        frame.render_widget(Paragraph::new(query), layout.query_area);
    }

    let Some(rows) = layout.rows_area else {
        return;
    };
    if search.matches.is_empty() {
        let status = if let Some(error) = &search.error {
            sanitize_control(error)
        ...
        frame.render_widget(
            Paragraph::new(status).style(Style::new().add_modifier(Modifier::DIM)),
            rows,
        );
        return;
    }
    ...
    let lines = search
        ...
        .flat_map(|(index, hit)| {
            workspace_search_match_lines(hit, &search.query, index == search.cursor, icons)
        })
        .collect::<Vec<_>>();
