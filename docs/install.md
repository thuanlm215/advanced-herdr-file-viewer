...
Full-text file/folder/workspace search (`F`) requires the `rg` executable on `PATH`. The rest of
the viewer works without it; pressing `F` shows a non-fatal requirement notice.
...
downloading a prebuilt binary or compiling from source, producing `./target/release/advanced-herdr-file-viewer`,
...
herdr plugin link /path/to/advanced-herdr-file-viewer
```
