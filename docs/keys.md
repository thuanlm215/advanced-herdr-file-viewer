...
Capital-letter bindings such as `C`, `D`, `F`, `G`, `H`, `L`, `N`, `O`, `R`, `W`, `Y`, and `Z` mean
...
`F`, `G`, `W`, `N`, `O`, `R`, `Z`, `?`, `H`/`L`, `J`/`K` in line-select mode, and `d`/`D` in the annotation
overview).
...
| **Wheel** over the content pane | Scroll it vertically; over the tree, move the whole viewport without changing selection |
